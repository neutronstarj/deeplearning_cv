...
def compute_distances_no_loops(x_train, x_test):
  ...
  A=x_train.reshape(num_train,-1) ## -1 means let pytorch reshape to appropuate columns
...
def predict_labels(dists, y_train, k=1):
  ...
  for i in range(indices.shape[1]):
    _,idx=torch.max(y_train[indices[:,i]].bincount(),dim=0) #bincount is counting the frequency of unique element
    y_pred[i] = idx
